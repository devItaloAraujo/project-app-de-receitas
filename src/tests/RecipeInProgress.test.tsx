...
beforeEach(async () => {
  ...
  await act(() => {
    render = renderWithRouter(<App />, { initialEntries: ['/meals/52772/in-progress'] });
  })
...
describe('Testes tela de receitas em progresso', async () => {
  ...
  test('Deve marcar uma receita como favorita', async () => {
    const favoriteBtnParent = screen.getByTestId('favorite-btn').parentElement as Element;
    expect(favoriteBtnParent.children.item(0)?.getAttribute('src')).toEqual('/src/images/whiteHeartIcon.svg');
  ...
  test('Deve remover as receitas favoritas', async () => {
    ...
    let favoriteBtn = screen.getByTestId('favorite-btn');
    expect(favoriteBtn.getAttribute('src')).toEqual('/src/images/whiteHeartIcon.svg');
  });
  ...
  test('Deve redirecionar para a página \'/done-recipes\' quando o botão \'Finish recipe\' for clicado', async () => {   
    ...
    for (let i = 0; i < listitems.length; i++) {
      ...
      await userEvent.click(listitem.children[0]);
    }
